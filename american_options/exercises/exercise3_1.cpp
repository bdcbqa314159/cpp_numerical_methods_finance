                             ...
                             BinLattice<double> &YTree)
{
    ...
    double option_value{}, x_pos{}, y_pos{};
    ...
    for (int n = N - 1; n >= 0; n--)
    {
        double discount_n = std::pow(discount, n);
        for (int i = 0; i <= n; i++)
        {
            double H_1 = PriceTree.GetNode(n + 1, i);
            double H_2 = PriceTree.GetNode(n + 1, i + 1);
            double S_1 = Model.S(n + 1, i);
            double S_2 = Model.S(n + 1, i + 1);

            option_value = (p * H_1 + q * H_2) * discount;
            x_pos = (H_2 - H_1) / (S_2 - S_1);
            y_pos = (option_value - S_1) * discount_n;

            PriceTree.SetNode(n, i, option_value);
            XTree.SetNode(n, i, x_pos);
            YTree.SetNode(n, i, y_pos);
        }
    }
